...
function LoginPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      const inviteToken = typeof window !== "undefined" ? window.sessionStorage.getItem("chronosquares-invite-token") : null;
      if (inviteToken) {
        window.sessionStorage.removeItem("chronosquares-invite-token");
  ...
  const handleGoogle = async () => {
    const inviteToken = window.sessionStorage.getItem("chronosquares-invite-token");
    const destination = inviteToken ? `/join/${inviteToken}` : "/app/dashboard";
    const result = await oauthClient.auth.signInWithOAuth("google", { redirect_uri: window.location.origin + destination });
